...
void BDData::loadBDFile(const std::string& filename)
{
   ...
   while (!bdFile.eof()) {
      ...
      if (firstChar == '#') {
         ...
         if (firstChrName == secondChrName && secondChrName != "" && abs((int)(firstPos - secondPos)) < 500) {
            continue;
         }
...
{
   int distance = 1000;
   if (abs((int)(first.PosA - first.PosA1) > distance)) {
      return false;
   }
   if (abs((int)(first.PosB - first.PosB1) > distance)) {
      return false;
   }
   if (abs((int)(second.PosA - second.PosA1) > distance)) {
      return false;
   }
   if (abs((int)(second.PosB - second.PosB1)) > distance) {
      return false;
   }
   float cutoff = 0.9;
...
void ModifyRP(std::vector <RP_READ> & Reads_RP)
{
   ...
   for (unsigned first = 0; first < Reads_RP.size(); first++) {
      ...
      if (Reads_RP[first].ChrNameA == Reads_RP[first].ChrNameB && abs((int)(Reads_RP[first].PosA - Reads_RP[first].PosB)) < 500) {
